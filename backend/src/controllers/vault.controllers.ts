...
const updatePasswordVault = AsyncHandler(
  async (req: Request, res: Response) => {
    ...
    const existingVault = await EncryptedPassword.findOne({
      userId: new mongoose.Types.ObjectId(userId),
    });
    if (!existingVault) {
      throw new ApiError(404, "Password vault not found");
    ...
    const updatedPasswordVault = await EncryptedPassword.findOneAndUpdate(
      {
        userId: new mongoose.Types.ObjectId(userId),
      },
      {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          "Password vault updated successfully",
